...
function textToken(content: string): Token {
    return { type: 'text', content } as unknown as Token;
}
...
describe('renderInline', () => {
    ...
    it('プレーンテキストをエスケープして返す', () => {
        expect(
            renderInline([textToken('Hello & "world"')] as unknown as Token[]),
        ).toBe('Hello &amp; &quot;world&quot;');
    });
    ...
    it('インライン数式を KaTeX ショートコードに変換', () => {
        const tokens = [textToken('before $x^2$ after')] as unknown as Token[];
        expect(renderInline(tokens)).toBe('before [katex]x^2[/katex] after');
    });

    it('1段落に複数のインライン数式を変換', () => {
        const tokens = [textToken('$a$ and $b$')] as unknown as Token[];
        expect(renderInline(tokens)).toBe(
    ...
    it('エスケープされた $ は通常文字として扱う', () => {
        const tokens = [textToken('price is \\$10')] as unknown as Token[];
        expect(renderInline(tokens)).toBe('price is $10');
    });

    it('display math 形式の $$...$$ はインライン数式として変換しない', () => {
        const tokens = [textToken('see $$x^2$$ end')] as unknown as Token[];

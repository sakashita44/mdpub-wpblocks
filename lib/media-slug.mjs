 ...
 * アップロード用ファイル名を算出する
 *
 * ファイル名中のスペース・特殊文字をハイフンに正規化し、
 * WordPress の sanitize_file_name と結果が一致するようにする。
 *
...
/**
 * ファイル名を WordPress 互換に正規化する
 * （拡張子は保持し、ベース部分のみサニタイズ）
 *
...
export function expectedSlug(imagePath, articleSlug) {
    ...
    // 例: 全角のみファイル名 "テスト.jpg" → slug が article slug と同一になる
